...
export default function Comments({comentarioProducto, calificacionProducto, nombreCliente, fechaValoracion }) {
  ...
  const renderStars = (rating) => {
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      stars.push(<FontAwesome key={i} name={name} size={24} color="#840606" />);
    }
    return stars;
  };
  ...
  return (
    <View style={styles.card}>
      <View style={styles.infoContainer}>
        <Text style={styles.textA}>{nombreCliente}</Text>
        <Text style={styles.textA}>{fechaValoracion}</Text>
      </View>
